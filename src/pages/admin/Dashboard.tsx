...
import { getAdminStats, MOCK_BOOKINGS } from "@/data/admin-data";
import { MOCK_TRIPS, formatPrice, PICKUP_POINTS } from "@/data/shuttle-data";
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={ClipboardList} label="Total Bookings" value={String(stats.totalBookings)} trend="12% vs last week" trendUp />
        <StatCard icon={DollarSign} label="Revenue" value={formatPrice(stats.totalRevenue)} trend="8% vs last week" trendUp />
        <StatCard icon={Users} label="Active Drivers" value={String(stats.activeDrivers)} />
        <StatCard icon={Bus} label="Trips Today" value={String(stats.tripsToday)} trend="2 more than yesterday" trendUp />
      </div>
