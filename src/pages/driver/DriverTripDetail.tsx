...
const makeStopIcon = (label: string, status: "completed" | "current" | "upcoming") =>
  L.divIcon({
    ...
    };border:2px solid ${status === "completed" ? "hsl(152,69%,40%)" : "hsl(217,91%,50%)"};display:flex;align-items:center;justify-content:center;font-size:${status === "current" ? 10 : 8}px;font-weight:700;color:${status === "upcoming" ? "hsl(217,91%,50%)" : "white"};font-family:sans-serif;box-shadow:0 2px 8px rgba(0,0,0,0.18)${status === "current" ? ";animation:pulse 2s infinite" : ""}">${status === "completed" ? "✓" : label}</div>`,
...
function LiveBusMarker({ position, bearing, followBus }: { position: [number, number]; bearing: number; followBus: boolean }) {
  ...
  return <Marker ref={markerRef} position={position} icon={icon}><Popup>🚌 Posisi driver saat ini</Popup></Marker>;
}

function CompletedRoute({ coords }: { coords: [number, number][] }) {
  if (coords.length < 2) return null;
  return <Polyline positions={coords} pathOptions={{ color: "hsl(152,69%,45%)", weight: 5, opacity: 0.8 }} />;
}
...
export default function DriverTripDetail() {
  ...
  useEffect(() => {
    const interval = setInterval(() => { setJitter([(Math.random() - 0.5) * 0.0002, (Math.random() - 0.5) * 0.0002]); }, 2000);
    return () => clearInterval(interval);
  }, []);
  ...
  useEffect(() => {
    ...
    const spd = currentStop > 0 && currentStop < pickupPoints.length ? Math.floor(25 + Math.random() * 15) : 0;
    const upsert = async () => {
      await supabase.from("driver_locations").upsert({
        trip_id: id, driver_id: "d1",
        latitude: pos[0] + jitter[0], longitude: pos[1] + jitter[1],
        bearing: b, speed: spd, current_stop_index: currentStop,
        updated_at: new Date().toISOString(),
      }, { onConflict: "trip_id" });
    };
    upsert();
  }, [id, busIndex, currentStop, jitter, smoothPath, pickupPoints.length]);
  ...
  const busPosition: [number, number] = [smoothPath[busIndex][0] + jitter[0], smoothPath[busIndex][1] + jitter[1]];
    ...
    <div className="mobile-container bg-background pb-6">
      <div className="sticky top-0 z-20 bg-card border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">{trip.departureTime} • {trip.vehiclePlate}</p>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-64 mx-4 mt-4 rounded-2xl overflow-hidden border border-border/50 shadow-lg relative">
        <MapContainer center={busPosition} zoom={14} scrollWheelZoom={false} zoomControl={false} attributionControl={false} style={{ height: "100%", width: "100%" }}>
          ...
          {pickupPoints.map((p, idx) => {
            ...
            return <Marker key={p.id} position={p.coords} icon={makeStopIcon(p.label, status)}><Popup><strong>{p.label}</strong> — {p.name}<br />{getPickupTime(trip.departureTime, p)}</Popup></Marker>;
          })}
          <LiveBusMarker position={busPosition} bearing={bearing} followBus={followBus} />
        </MapContainer>
        <button onClick={() => setFollowBus(!followBus)} className={`absolute bottom-3 right-3 z-[1000] w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-colors ${followBus ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground border border-border"}`}>
      ...
      <AnimatePresence>
        {tripStatus === "running" && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mx-4 mt-3 flex items-center gap-2">
            <div className="flex-1 flex items-center gap-2 bg-primary/10 rounded-xl px-3 py-2"><Gauge className="w-4 h-4 text-primary" /><span className="text-sm font-bold text-primary">{speed} km/h</span></div>
            <div className="flex-1 flex items-center gap-2 bg-secondary/10 rounded-xl px-3 py-2"><Clock className="w-4 h-4 text-secondary" /><span className="text-sm font-bold text-secondary">~{etaMinutes} min</span></div>
            <div className="flex-1 flex items-center gap-2 bg-[hsl(var(--shuttle-warning))]/10 rounded-xl px-3 py-2"><MapPin className="w-4 h-4 text-[hsl(var(--shuttle-warning))]" /><span className="text-sm font-bold text-[hsl(var(--shuttle-warning))]">{pickupPoints[currentStop]?.label || "—"}</span></div>
          </motion.div>
        )}
      </AnimatePresence>

      <div className="mx-4 mt-3 flex items-center gap-3 text-xs text-muted-foreground">
        <div className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-primary" /><span>{pickupPoints.length} halte</span></div>
        <span>•</span><span>{passengers.length} penumpang</span>
        <span>•</span><span>Stop {Math.min(currentStop + 1, pickupPoints.length)}/{pickupPoints.length}</span>
      </div>
